...
def test_close_position_at_stop_scores_immediately_and_keeps_horizon_honest():
    led = Ledger(":memory:")
    r = decide("SOL", RecordedRyoClient(FIXTURES), make_llm(action="long", p=0.7), led)  # long at RYO's recorded price, stop 2 ATR below
    ...
    exit_at = r.trade.stop_price - 0.5  # the stop was crossed
    out = close_position(r.id, led, exit_at, "exchange_median:3_sources", "stopped")
    assert out.closed_reason == "stopped" and out.horizon_reached is False and out.went_up is False
    assert out.trade_result_usd == pytest.approx((exit_at - r.trade.entry_price) * r.trade.size_units, abs=0.01)
    assert out.price_now_source.startswith("exchange_median")
    assert led.unresolved() == [] and role_scores(led)["judge"]["brier_mean"] == pytest.approx(0.49)
    blocked = decide("SOL", RecordedRyoClient(FIXTURES), make_llm(action="no_trade", p=0.5), led, use_cache=False)
