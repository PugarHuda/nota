...
class AnthropicLLM:
    def __init__(self, model: str | None = None, max_tokens: int | None = None):
        ...
        self.model = model or os.environ.get("NOTA_MODEL", DEFAULT_MODEL)
        # NOTA_MAX_TOKENS lets a small prepaid balance (e.g. OpenRouter) fit; outputs are short JSON
        self.max_tokens = max_tokens or int(os.environ.get("NOTA_MAX_TOKENS", "16000"))
    ...
    def complete_json(self, system: str, user: str, schema: type[T]) -> T:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "raise NOTA_MAX_TOKENS (needs ~2500 per council call)"
            ) from exc
        if response.stop_reason == "max_tokens":
            raise RuntimeError(f"model hit max_tokens={self.max_tokens}; raise NOTA_MAX_TOKENS")
...
class OpenAICompatLLM:
    ...
    Env: OPENAI_BASE_URL, OPENAI_API_KEY, NOTA_MODEL, NOTA_MAX_TOKENS. ponytail: plain httpx, no SDK.
    """
